...
const DoctorSpecialty = () => {
    ...
    useEffect(() => {
        const fetchDoctorsBySpecialty = async () => {
            ...
            try {
                ...
                const response = await api.get(`/api/doctor/specialty/${id}`);

                if (response.data && response.data.errCode === 0) {
                    setDoctors(response.data.data);

                    // Lấy tên chuyên khoa để hiển thị lên tiêu đề (nếu có dữ liệu)
                    // Giả định API trả về mảng bác sĩ, trong mỗi bác sĩ có thông tin specialty
                    if (
                        response.data.data.length > 0 &&
                        response.data.data[0].specialty
                    ) {
                        setSpecialtyName(response.data.data[0].specialty.name);
                    }
                } else {
                    setDoctors([]);
                }
    ...
    // Xử lý tìm kiếm trên danh sách đã tải về
    ...
    // Hàm chuyển hướng sang trang chi tiết bác sĩ
            ...
            <div className='container mx-auto max-w-6xl'>
                ...
                <div className='mb-8 text-center'>
                    ...
                    <p className='text-gray-500 mb-6'>
                        Danh sách các bác sĩ uy tín thuộc chuyên khoa này
                    </p>
                    ...
                    <div className='relative max-w-lg mx-auto'>
                        ...
                        <input
                            ...
                            placeholder='Tìm kiếm bác sĩ trong khoa...'
                            value={searchTerm}
                ...
                {/* Grid hiển thị danh sách bác sĩ - Cấu trúc giống AllDoctor */}
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {filteredDoctors.map((doc) => (
                        <div
                            key={doc.id}
                            // Sự kiện click vào thẻ để chuyển hướng
